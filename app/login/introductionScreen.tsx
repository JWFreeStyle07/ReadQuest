...
import { getCurrentUserId, getUserProfile } from "../../firebase/authService";
...
const IntroductionScreen = () => {
    ...
    const fetchUserType = async () => {
      try {
        ...
        if (!userId) {
          ...
          router.replace("../../login/startScreen.tsx");
          return;
        }

        const userProfile = await getUserProfile(userId);
        const fetchedUserType = userProfile.userType || 'student'; // Default to student if not set
        ...
        setIsLoading(false);
        // Default to student if error occurs
        setUserType('student');
      }
    };
  ...
  const handleContinue = () => {
    ...
    Animated.parallel([wiggle, scale]).start(() => {
      // Navigate based on user type after animation
      setTimeout(() => {
        if (userType === 'teacher') {
