...
//   const normalizedTitle = decodeURIComponent(String(title || ""));
  
//   const story = referenceTexts.find(
//     (item) => item.title.toLowerCase() === normalizedTitle.toLowerCase()
//   );
  
//   // Use the actual story title from referenceTexts for vocabulary lookup
//   const storyVocab = story ? vocabularies[story.title] || {} : {};
...
export default function VocabularySupport() {
  ...
  // 🎯 HARDCODED: Always use "Counting the Hours" story
  const hardcodedTitle = "Counting the Hours";
  
  // Use hardcoded title if no parameter is passed, otherwise use the parameter
  const targetTitle = title ? decodeURIComponent(String(title)) : hardcodedTitle;
    ...
    return (
      <View style={styles.scrollContainer}>
        <View style={styles.card}>
          ...
          <Text>Received title param: {String(title || 'undefined')}</Text>
          <Text>Target title: {targetTitle}</Text>
          <Text style={{ marginTop: 10 }}>Available stories:</Text>
