...
// const UniversalStoryScreen = () => {
//   const router = useRouter();
//   const params = useLocalSearchParams<{ title?: string }>();
//   const storyTitle = params.title ? decodeURIComponent(String(params.title)) : 'Counting The Hours';
//   const REFERENCE_TEXT = STORY_DATA[storyTitle] || STORY_DATA['Counting The Hours'];
...
//   const saveRecordingLocally = async (uri: string) => {
//     try {
//       const recordingData: LocalRecording = {
...
//       const recordings: LocalRecording[] = existingRecordings ? JSON.parse(existingRecordings) : [];
//       recordings.push(recordingData);
      
//       await AsyncStorage.setItem('localRecordings', JSON.stringify(recordings));
//       console.log('✅ Recording saved locally:', recordingData.id);
//     } catch (error) {
...
//   // Save to Firebase (only when online and user is logged in)
//   const saveResultToFirebase = async (pronunciationScore: number, readerLevel: string) => {
//     try {
//       const currentUser = auth.currentUser;
      
//       if (!currentUser || !isOnline) {
//         console.log('ℹ️ Skipping Firebase save (offline or no user)');
//         return;
//       }
...
//     if (!isOnline) {
//       Alert.alert('Offline Mode', 'Analysis requires internet connection. Your recording has been saved locally and will be available for analysis when you\'re back online.');
//       return;
//     }
...
//       Alert.alert('Error', 'Failed to analyze pronunciation. Please check your internet connection and try again.');
//     } finally {
...
//           const adjustedPosition = firstWordOffset + (audioProgress * timestampRange);

//           const currentWordIndex = wordsWithTimestamps.findIndex((word: any, index: number) => {
//             const nextWord = wordsWithTimestamps[index + 1];
            
//             if (nextWord) {
//               return adjustedPosition >= word.offset && adjustedPosition < nextWord.offset;
//             } else {
//               return adjustedPosition >= word.offset;
//             }
//           });
...
//           onPress={() => router.push('../../offline/offlineRecordings' as any)}
//         >
...
//         {/* Watch Playback Button - Show after recording stops */}
...
// export default UniversalStoryScreen;
import { MaterialCommunityIcons } from '@expo/vector-icons';
import AsyncStorage from '@react-native-async-storage/async-storage';
...
const UniversalStoryScreen = () => {
  ...
  const saveResultToFirebase = async (
    ...
    try {
      ...
      // Calculate miscues based on error types in the result
      const miscues = pronunciationResult?.words.filter(
        word => word.errorType !== 'None'
      ).length || 0;
